 ...
 * 
 * For nginx server installation and production read /script/install.sh . Note that we use Certbot to grant SSL certificate.
 * 
 */
const config = {
  api: {
    host: 'https://explorer.fdreserve.com', // ex: 'https://explorer.bulwarkcrypto.com' for nginx (SSL), 'http://IP_ADDRESS' 
    port: '3000', // ex: Port 3000 on prod and localhost
    portWorker: '443', // ex: Port 443 for production(ngingx) if you have SSL (we use certbot), 3000 on localhost or ip
    prefix: '/api',
    timeout: '5s'
  },
  coinDetails: {
    ...
    shortName: 'FDR',
    displayDecimals: 2,
    longName: 'French Digital Reserve',
    ...
    websiteUrl: 'https://www.fdreserve.com/',
    masternodeCollateral: 1000, // MN ROI% gets based on this number. If your coin has multi-tiered masternodes then set this to lowest tier (ROI% will simply be higher for bigger tiers)
    cashnodeCollateral: 10000,
    reservenodeCollateral: 50000
  },
  // Number Of NFT Nodes
  nftNodes: {
      Rnodes: 1,
  ...
  offChainSignOn: {
    enabled: true,
    signMessagePrefix: 'FDRSIGN-' // Unique prefix in "Message To Sign" for Off-Chain Sign On
  },

  // Add any important block counting down in this array
  blockCountdowns: [
    {
      block: 3664800, // What block are we counting down to?
      beforeTitle: 'Next Halving', // What do we show before the block number is hit?
      afterTitle: 'Halving Since' // What do we show after the block number is hit?
    }
  ],
  ...
  social: [
    {
      name: 'FD_Reserve', // Unique name of the social widget
      type: SocialType.Twitter, // What type of social widget is it?
...
//      }
  ...
  community: {
    ...
    highlightedAddresses: [
      { label: 'Dev Fee', address: 'XXXXXXXXXXXXXXXXXXXXXXX' }, // Dev Fees Wallet
      { label: 'NFT Full Reserve Node #001', address: 'XXXXXXXXXXXXXXXXXXXXXXX' }, // ReserveNode NFT 001
      { label: 'NFT Full Cash Node #001', address: 'XXXXXXXXXXXXXXXXXXXXXXX' }, // CashNode NFT 001
      { label: 'NFT Full Security Node #001', address: 'XXXXXXXXXXXXXXXXXXXXXXX' }, // // Secure Node NFT 001
    ],
  ...
  addressWidgets: {
    ...
    'COINBASE': {
      // Adds a new label metadata address
      carverAddressLabelWidget: {
        label: 'Coinbase (Premine & POS) 💎',
        title: 'This address was active during Proof Of Stake (POS) phase to distribute rewards to stakers & masternode owners.'
      }
    },
    'MN': {
      // Adds a new label metadata address
      carverAddressLabelWidget: {
        ...
        title: 'Each block contains a small portion that is awarded to masternode operators that lock 1000 / 10 000 / 50 000 FDR. Masternodes contribute to the network by handling certain coin operations within the network.'
      }
    },
    'POW': {
      // Adds a new label metadata address
      carverAddressLabelWidget: {
        ...
        title: 'FDR started as a Proof Of Stake & Masternode coin. Blocks would be mined by Staking and be rewarded for keeping up the network.'
      }
    },
    'POS': {
      // Adds a new label metadata address
      carverAddressLabelWidget: {
        ...
        title: 'Inputs that are over 1 FDR can participate in network upkeep. Each block (~90 seconds) one of these inputs is rewarded for keeping up the network.'
      }
    },
  },
